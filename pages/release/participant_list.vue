<template>
	<view>
		<!-- <u-navbar :background="{ background: '#ff0000' }" title-color="#fff" :is-back="true" :custom-back="isback" :title-bold="true" back-icon-color="#fff" title="订单查看"></u-navbar> -->
	...
	methods: {
		...
		isback(){
			...
			if (pages[pages.length - 2]) {
				...
				this.$u.route({url:'pages/tabbar/activity/activity'})
			}
		},
		jieliu(){
			let Interval = null
		...
		async getSearchData(e){
			await db
				...
				.where({'info.value': new RegExp(e, 'ig'),'dataList._id':this.id }).orderBy("register_date","desc")
				.limit(500)
		...
		async loaddata(substate) {
			...
			if(substate == '全部'){
				...
				await db
					.collection('enroll_info')
					.where({ 'dataList._id':this.id }).orderBy("register_date","desc")
					.limit(this.pagesize)
					.skip((this.pageindex - 1) * this.pagesize)
					.get()
					.then(res => {
				...
				await db
					.collection('enroll_info')
					.where({  state: this.list[this.current].name,'dataList._id':this.id }).orderBy("register_date","desc")
					.limit(this.pagesize)
					.skip((this.pageindex - 1) * this.pagesize)
					.get()
					.then(res => {
