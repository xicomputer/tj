<template>
    ...
    <view class="top-head">
      <!-- <image class="topimage" :src="statisticsList.headImgUrl"></image> -->
			<u-avatar class="topimage" :src="statisticsList.headImgUrl"></u-avatar>
      <text>{{ statisticsList.nickName }}</text>
    ...
    <view class="box-list u-border-bottom u-border-top">
      ...
      <view>
        <text>{{ statisticsList.statistics.tjCount }}</text>
        <text>推荐官</text>
      </view>
      <view>
        <text>{{ statisticsList.statistics.totalSupplierCount}}</text>
        <text>供货商家</text>
      </view>
    </view>
		<view class="box-img">
	...
	export default {
		...
		onLoad(option) {
			...
			uni.getStorage({
				...
				success:res=>{
					if(option.dealerId){
						//链接进来
						this.applyState(option.dealerId)
            this.applystatisticsList(option.dealerId)
					}else{
							this.pjseo(scene)
					}
				},
		...
		methods: {
			getToken(indexEvent){
                ...
                console.log('999',that.$store.state.tokenobj)
									uni.setStorage({
        ...
        console.log(11111)
					...
					this.$u.api.dealerApply(param).then(e=>{

						this.applyState(this.dealerId)
					}).catch(err=>{
			...
			applyState(id){		//获取申请状态
				this.$u.api.applytystatic(id).then(e=>{
					if(e){
			...
			pjseo(send){			//破解二维码
				...
				this.$u.api.getAppletCodeScene(send).then(e=>{
					...
					this.applyState(this.dealerId)
          this.applystatisticsList(e.dealerId)
				}).catch(err=>{
					console.log(err)
				})
			},
      jumpIndex(){
		  // uni.navigateTo({
		  // 	url:"/pages/goods/newgoods_recommend/new_shop"
		  // })
        uni.switchTab({
      ...
      applystatisticsList(dealerId){
        this.$u.api.applystatisticsList(dealerId).then(e=>{
